...
bool j1App::Awake()
{
	...
	if(ret == true)
	{
		...
		while(item != modules.end() && ret == true) //What if here instead of item != modules.end() we place item._Ptr != NULL?
		{
			
			ret = (*item)->Awake(config.child((*item)->name.data())); //& What if instead of (*item) we put item.Ptr.MyVal?
			item = next(item); //What if instead of this, we use item.Ptr = item.Ptr.next?

			/*ret = item->data->Awake(config.child(item->data->name.data()));
			item = item->next;*/
			//CHANGE HERE IF WRONG!
		}
	}
...
bool j1App::SavegameNow() const
{
	...
	std::list<j1Module*>::const_iterator item = modules.begin(); //Cause function is CONST
