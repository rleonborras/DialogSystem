...
bool DialogSystem::Start() {
	...
	dialogB = createDialog(DialogConfig_file, 1);


	//REMEMBER TO ADD THE DIALOGS IN ORDER TO CLEAN IT UP
	Dialog_List.push_back(dialogA);
	Dialog_List.push_back(dialogB);
...
void Dialogue::Draw() {
	...
	if (currentNode->getPosition_Spawn() == TOP_LEFT || currentNode->getPosition_Spawn() == DOWN_LEFT) {
		
		App->render->Blit(Dialog_Textures, currentNode->dialogChart.w / scale, currentNode->dialogChart.y / scale, &App->dialog->GeneraldialogChartEnd, 1, scale);
		App->render->Blit(Dialog_Textures, currentNode->dialogChart.x, currentNode->dialogChart.y / scale - (temp_Character.characterRect.h - currentNode->dialogChart.h / scale), &temp_Character.characterRect, 1, scale);
	}
	else if (currentNode->getPosition_Spawn() == TOP_RIGHT || currentNode->getPosition_Spawn() == DOWN_RIGHT) {

		App->render->Blit(Dialog_Textures, currentNode->dialogChart.x / scale - App->dialog->GeneraldialogChartEnd.w + 1, currentNode->dialogChart.y / scale, &App->dialog->GeneraldialogChartEnd, 1, scale, SDL_FLIP_HORIZONTAL);
		App->render->Blit(Dialog_Textures, (currentNode->dialogChart.x + currentNode->dialogChart.w) / scale - temp_Character.characterRect.w, currentNode->dialogChart.y / scale - (temp_Character.characterRect.h - currentNode->dialogChart.h / scale)
			, &temp_Character.characterRect, 1, scale, SDL_FLIP_HORIZONTAL);
	}
